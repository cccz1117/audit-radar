...
- **无增量信息的评论/观点文章**（对 48 小时以上旧事件的读后感、社论、纯情绪评论。
  判据看摘要而非标题：无新事实、新数据、新进展、新涉事方 → 拒绝；
  有实质增量的跟进报道（如披露"持续数周未被发现"的新细节）**必须保留**）
...
> **保留配额**：keep 为 yes/strong 的条目每批最多 25 条；超过时只保留分数最高的，其余降为 no 输出。
...
4. 总分 < 20 或红线 → 拒绝
5. 可选：先输出一行汇总（保留/拒绝数量概述），然后输出 JSON 数组。**JSON 数组之后禁止输出任何内容**
